...
class ChessRankingApp:
    ...
    async def get_ratings(self, player, perf_type: str, days: int):
        ...
        rating_histories = await loop.run_in_executor(
            None, lambda: PlayerRatingHistoryService.get_rating_history(player.username)
        )
    ...
    async def process_players_rating_data(
        self, perf_type: str, quantity: int, days: int
    ):
        ...
        try:
            ...
            return []
        loop = asyncio.get_event_loop()
        rating_histories = await asyncio.gather(
            *[
                loop.run_in_executor(
                    None,
                    lambda p=player: PlayerRatingHistoryService.get_rating_history(
                        p.username
                    ),
                )
